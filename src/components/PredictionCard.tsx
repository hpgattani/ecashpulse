...
const PredictionCard = ({ prediction, index, livePrice }: PredictionCardProps) => {
  ...
  const isMultiOption = prediction.isMultiOption && prediction.outcomes && prediction.outcomes.length > 0;
        ...
        <div className="p-4 md:p-5">
          {isMultiOption ? (
            ...
            <div className="space-y-2 mb-4">
              {prediction.outcomes!.slice(0, 4).map((outcome, i) => (
                <div 
                  key={outcome.id}
                  className="flex items-center justify-between p-2 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors cursor-pointer group/outcome"
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex gap-1 opacity-0 group-hover/outcome:opacity-100 transition-opacity">
                      <Button variant="yes" size="sm" className="h-6 px-2 text-xs">Yes</Button>
                      <Button variant="no" size="sm" className="h-6 px-2 text-xs">No</Button>
                    </div>
                  </div>
                </div>
              ))}
