...
const BetModal = ({ isOpen, onClose, prediction, position, selectedOutcome }: BetModalProps) => {
  ...
  // --------------------------------------------------
  // ✅ CORRECT PARIMUTUEL PAYOUT LOGIC
  // --------------------------------------------------
  const betAmountNum = parseFloat(betAmount) || 0;

  const yesPool = Number(prediction.yes_pool || 0);
  const noPool = Number(prediction.no_pool || 0);

  const opposingPool = betPosition === "yes" ? noPool : yesPool;

  const totalPoolBefore = yesPool + noPool;

  let totalPayout = betAmountNum;
  let profit = 0;

  if (opposingPool > 0 && totalPoolBefore > 0) {
    profit = (betAmountNum * opposingPool) / totalPoolBefore;
    totalPayout = betAmountNum + profit;
  }

  const winMultiplier = betAmountNum > 0 ? totalPayout / betAmountNum : 1;
  ...
  const recordBet = useCallback(
    async (txHash?: string) => {
      ...
      const betAmountSatoshis = Math.round(betAmountNum * 100);
            ...
            <div className="glass-card glow-primary p-4 sm:p-6 w-full">
              ...
              <div className="mt-4 p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-bold text-emerald-400">{totalPayout.toFixed(2)} XEC</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span>{t.profit}</span>
                  <span className="font-bold text-emerald-400">+{profit.toFixed(2)} XEC</span>
                </div>
                <div className="text-xs text-muted-foreground mt-1">Multiplier: {winMultiplier.toFixed(2)}x</div>
              </div>
