...
async function buildTransactionWithEcashAgora(
  ...
  try {
    ...
    const { Script, fromHex, toHex, shaRmd160, Tx, TxBuilder, P2PKHSignatory, ALL_BIP143 } = await import('https://esm.sh/ecash-agora@0.2.1');
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const txResult = await buildTransactionWithEcashAgora(
      escrowPrivateKey,
      ESCROW_ADDRESS,
      recipients
    );
