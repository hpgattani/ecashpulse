// Shared crypto primitives for eCash transaction building
// Used by: send-payouts, send-refund, send-bonus-payouts, sweep-escrow, resolve-game-leaderboards
...
export function cashAddrToHash160(address: string): Uint8Array | null {
  try {
...
export function createP2PKHScript(hash160Bytes: Uint8Array): Uint8Array {
  const script = new Uint8Array(25);
  script[0] = 0x76; // OP_DUP
  script[1] = 0xa9; // OP_HASH160
  script[2] = 0x14; // Push 20 bytes
  script.set(hash160Bytes, 3);
  script[23] = 0x88; // OP_EQUALVERIFY
  script[24] = 0xac; // OP_CHECKSIG
  return script;
}

export function createCashtabMessageScript(message: string): Uint8Array {
...
export async function getPublicKey(privateKey: Uint8Array, compressed: boolean): Promise<Uint8Array> {
  return secp.getPublicKey(privateKey, compressed);
}

export async function signECDSA(messageHash: Uint8Array, privateKey: Uint8Array): Promise<Uint8Array> {
...
export async function bip143Sighash(
  inputs: TxInput[],
  outputs: TxOutput[],
  inputIndex: number,
  hashType: number
