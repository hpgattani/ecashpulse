...
// Chronik client for eCash
const chronik = new ChronikClient(['https://chronik.e.cash']);
const ESCROW_ADDRESS = 'ecash:qz6jsgshsv0v2tyuleptwr4at8xaxsakmstkhzc0pp';

// Bonus amount in satoshis (1000 XEC = 100,000 satoshis)
const BONUS_AMOUNT_SATS = 100000;

// ==================== Crypto Primitives ====================
// (Same as send-payouts - SHA256, RIPEMD160, hash functions)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Get ALL users who have ever placed a confirmed bet on the platform
    ...
    if (!allBettors || allBettors.length === 0) {
      return new Response(
        JSON.stringify({ success: true, message: 'No bettors found', paid: 0 }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    // Deduplicate by user address
    const uniqueAddresses = new Map<string, string>(); // address -> userId
    for (const bet of allBettors) {
      const address = (bet.users as any).ecash_address;
      if (address && !uniqueAddresses.has(address)) {
        uniqueAddresses.set(address, bet.user_id);
      }
    }

    const recipients = Array.from(uniqueAddresses.entries()).map(([address, userId]) => ({
      address,
      userId,
      amount: BONUS_AMOUNT_SATS,
    }));
    ...
    console.log(`Total bonus: ${totalBonus} sats (${totalBonus / 100} XEC), Fee: ~${estimatedFee}, Available: ${totalAvailable}`);
    ...
    const bonusMessage = '🎉 Bonus from eCash Pulse! A market resolved YES!';
    const opReturnScript = createCashtabMessageScript(bonusMessage);
    outputs.push({
      value: 0n,
    ...
    console.log(`Building bonus tx: ${inputs.length} inputs, ${outputs.length} outputs`);

    // Build and sign transaction
    const rawTx = await buildSignedTransaction(inputs, outputs, privateKey, compressed);
    console.log(`Bonus transaction built: ${rawTx.length} bytes`);
    ...
    // Log the bonus payout in audit
    await supabase.from('bet_audit_log').insert({
      event_type: 'bonus_payout_sent',
      prediction_id,
      metadata: {
        reason: trigger_reason || 'YES resolution',
        recipients_count: recipients.length,
        amount_per_recipient: BONUS_AMOUNT_SATS,
    ...
    return new Response(
      JSON.stringify({ 
        success: true,
        message: `Bonus payouts sent to ${recipients.length} bettors`,
        txid,
        recipients: recipients.length,
        amount_per_recipient: BONUS_AMOUNT_SATS / 100, // XEC
        total_amount: totalBonus / 100, // XEC
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
    ...
    return new Response(
      JSON.stringify({ 
        error: error instanceof Error ? error.message : 'Failed to send bonus payouts',
      }),
