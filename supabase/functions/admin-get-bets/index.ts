...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: bets, error } = await supabase
      .from('bets')
      .select('id, amount, position, status, tx_hash, created_at, confirmed_at, user_id, prediction_id, users(ecash_address), predictions(title, category)')
    ...
    return new Response(JSON.stringify({ bets: bets ?? [] }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
    console.error('admin-get-bets unexpected error:', error);
    return new Response(
